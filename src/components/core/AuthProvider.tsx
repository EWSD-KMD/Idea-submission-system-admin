...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const fetchUserProfile = useCallback(
    async (id: number) => {
      ...
      try {
        const response: UserProfileResponse = await serverFetch(`api/users/${id}`)
  ...
  useEffect(() => {
    if (!loading && authState.user?.id && (!userProfile || userProfile.id !== authState.user.id)) {
  ...
  const hasPermission = useCallback(
    (menuName: string, permission: string): boolean => {
      if (!userProfile || !userProfile.role || !userProfile.role.menus) return false

      const menu = userProfile.role.menus.find((m) => m.name.toLowerCase() === menuName.toLowerCase())
